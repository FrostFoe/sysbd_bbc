...
function get_bbc_data() {
    ...
    foreach ($sections as $section) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM articles WHERE section_id = ? ORDER BY created_at DESC");
        ...
        foreach ($articles as $article) {
            ...
            // Fetch comments
            $stmt = $pdo->prepare("SELECT user_name as user, text, time FROM comments WHERE article_id = ? ORDER BY created_at DESC");
            $stmt->execute([$article['id']]);
            $articleData['comments'] = $stmt->fetchAll();

            // Fetch culprit profile
            $stmt = $pdo->prepare("SELECT * FROM culprit_profiles WHERE article_id = ?");
            $stmt->execute([$article['id']]);
            $profile = $stmt->fetch();
            ...
            if ($profile) {
                ...
                // Fetch timeline
                $stmt = $pdo->prepare("SELECT year, event FROM culprit_timeline WHERE profile_id = ?");
                $stmt->execute([$profile['id']]);
                $profileData['timeline'] = $stmt->fetchAll();

                // Fetch associates
                $stmt = $pdo->prepare("SELECT name, role FROM culprit_associates WHERE profile_id = ?");
                $stmt->execute([$profile['id']]);
