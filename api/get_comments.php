...
switch($sort) {
    case 'oldest':
        ...
        break;
    case 'helpful':
        $orderBy = "(SELECT COUNT(*) FROM comment_votes WHERE comment_id = c.id AND vote_type = 'upvote') DESC, c.created_at DESC";
        break;
    case 'discussed':
        $orderBy = "(SELECT COUNT(*) FROM comments WHERE parent_comment_id = c.id) DESC, c.created_at DESC";
        break;
}
...
foreach ($rawComments as $c) {
    ...
    foreach ($rawReplies as $r) {
        ...
        $timeStr = "ঠিক এখন";
        if ($timeAgo->i > 0) $timeStr = $timeAgo->i . " মিনিট আগে";
        else if ($timeAgo->h > 0) $timeStr = $timeAgo->h . " ঘন্টা আগে";
        else if ($timeAgo->d > 0) $timeStr = $timeAgo->d . " দিন আগে";
