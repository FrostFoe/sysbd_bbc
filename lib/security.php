...
function sanitize_html($html) {
    ...
    $allowed_tags = ['p', 'b', 'strong', 'i', 'em', 'u', 'a', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'br', 'img', 'blockquote', 'span', 'div'];
    ...
    foreach ($nodes as $node) {
        if (!in_array($node->nodeName, $allowed_tags)) {
            ...
            if (in_array($node->nodeName, ['script', 'style', 'iframe', 'object', 'embed', 'applet', 'meta', 'link'])) {
    ...
    $process_node = function($node) use (&$process_node, $allowed_tags, $allowed_attrs) {
    ...
    for ($i = $all->length - 1; $i >= 0; $i--) {
        $node = $all->item($i);
        if (in_array($node->nodeName, ['script', 'style', 'iframe', 'object', 'embed', 'applet', 'link'])) {
            $node->parentNode->removeChild($node);
