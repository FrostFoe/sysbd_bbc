...
if (!$commentId || !in_array($voteType, ['upvote', 'downvote'])) {
    send_response(["error" => "Missing or invalid fields (commentId, voteType)"], 400);
    exit;
}
...
if ($existingVote) {
    if ($existingVote['vote_type'] === $voteType) {
        ...
        // Change vote
        $updateStmt = $pdo->prepare("UPDATE comment_votes SET vote_type = ? WHERE id = ?");
...
send_response([
    ...
    "score" => ((int)($counts['upvotes'] ?? 0) - (int)($counts['downvotes'] ?? 0))
]);
?>
