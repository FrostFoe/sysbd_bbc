        ...
        <table class="w-full text-left border-collapse responsive-table">
            ...
            <tbody class="divide-y divide-border-color">
                ...
                <tr class="hover:bg-muted-bg transition-colors">
                    <td class="p-4 font-bold text-sm"><?php echo htmlspecialchars(
                        $c["user_name"],
                    ); ?></td>
                    <td class="p-4 text-sm max-w-md truncate"><?php echo htmlspecialchars(
                        $c["text"],
                    ); ?></td>
                    <td class="p-4 text-xs text-muted-text">
                        <?php
                        $displayTitle = !empty($c["title_en"])
                            ? $c["title_en"]
                            : $c["title_bn"];
                        $langParam = !empty($c["title_en"]) ? "en" : "bn";
                        ?>
                        <a href="../read/index.php?id=<?php echo $c[
                            "article_id"
                        ]; ?>&lang=<?php echo $langParam; ?>" target="_blank" class="hover:text-bbcRed">
                            <?php echo htmlspecialchars($displayTitle); ?>
                        </a>
                    </td>
                    <td class="p-4 text-xs text-muted-text"><?php echo date(
                        "M d, H:i",
                        strtotime($c["created_at"]),
                    ); ?></td>
                    <td class="p-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button onclick="deleteComment(<?php echo $c[
                                "id"
                            ]; ?>)" class="text-red-500 hover:text-red-700 p-2 rounded hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Delete">
